...
void BasicWheelController::purePursuitFollow(VectorArr arr, bool isNeg){
  ...
  while(timeIn * sleepTime < maxTimeIn){
    ...
    double targetVel = speed;
    double rightExtra;
    {
      double targetRobotVel = pctToReal(targetVel);
      rightExtra = realToPct(travelCurvature * (trackWidth + 3.0) * targetRobotVel / 2.0);
    }
    //If it's supposed to be on the path in 100 ms, don't turn the robot
    if(bezier[indexIn100].dist2D(estimateIn100) < pathRadius){
      rightExtra = 0;
    ...
    //Mindblowing lines right here
    //Move the robot
    moveRight(abs(speed - rightExtra), isNeg ? reverse : fwd);
    moveLeft(abs(speed + rightExtra), isNeg ? reverse : fwd);
    lastPos = botPos();
    //Sleep (WOW, HE'S A GENIUS)
    s(sleepTime);
